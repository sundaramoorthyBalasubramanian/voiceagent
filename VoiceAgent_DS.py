...
class PigLatinVoiceAgent:
    ...
    def respond(self, input_audio, voice, speed, tone, background_noise):
        ...
        if background_noise != "None":
            noise_file = f"assets/{background_noise.lower()}.mp3"  # Assume noise files are in an 'assets' folder
...
# Gradio interface
with gr.Blocks() as voiceagentui:
    ...
    with gr.Row():
        mic_input = gr.Audio(sources=["microphone"], type="filepath", label="Record your voice", interactive=True)
